...
可以在 [七牛云存储开发者网站后台](https://dev.qiniutek.com/buckets) 进行相应的域名绑定操作，域名绑定成功后，若您将 bucket 设为公用（public）属性，则可以用如下方式对上传的文件进行访问：
...
注意：使用自定义域名，需要您将此域名的ICP备案号和企业营业执照附件发送到 <support@qiniutek.com>，七牛工作人员会完成后续处理。
